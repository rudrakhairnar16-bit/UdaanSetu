    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 24, overflowX: 'auto' }}>
        {pipelineSteps.map((step, i) => (
          <div key={step} style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ padding: '12px 20px', borderRadius: 10, fontWeight: 700, fontSize: 13, background: ['#eff6ff', '#f0fdf4', '#fff7ed', '#fdf4ff'][i], color: ['#1e40af', '#166534', '#9a3412', '#86198f'][i], whiteSpace: 'nowrap' }}>
              {step.charAt(0).toUpperCase() + step.slice(1)}
              <span style={{ marginLeft: 8, fontWeight: 400, opacity: .7 }}>{dash.pipeline?.[step]?.total || 0}</span>
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <div style={{ padding: '12px 20px', borderRadius: 10, fontWeight: 700, fontSize: 13, background: '#fef2f2', color: '#991b1b', whiteSpace: 'nowrap' }}>Impact</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 24 }}>
        {Object.entries(dash.counts).map(([k, v]) => (
          <div className="stat-card" key={k}><span className="label">{k.replace('_', ' ')}</span><span className="value">{String(v)}</span></div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }} className="two-col">
        ...
        <div className="card">
          ...
          {dash.recent.length === 0 ? (
            ...
            <div key={r.id} style={{ padding: '10px 0', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{r.title}</div>
