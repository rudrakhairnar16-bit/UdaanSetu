...
function StageBadge({ stage }: { stage: string }) {
  const map: Record<string, string> = { 'available': 'badge-green', 'open': 'badge-green', 'closed': 'badge-gray', 'submitted': 'badge-blue', 'under review': 'badge-yellow', 'approved': 'badge-green' };
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <h2>New {kind.charAt(0).toUpperCase() + kind.slice(1)}</h2>
            ...
            <div key={r.id} className="card" style={{ cursor: 'pointer' }} onClick={() => setDetail(r)}>
              ...
              {r.meta.expertise && (
                <div style={{ marginTop: 10, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {r.meta.expertise.map((e: string) => <span key={e} className="badge badge-gray" style={{ fontSize: 11 }}>{e}</span>)}
                </div>
              )}
              {r.meta.services && (
                <div style={{ marginTop: 10, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {r.meta.services.map((s: string) => <span key={s} className="badge badge-blue" style={{ fontSize: 11 }}>{s}</span>)}
                </div>
              )}
            </div>
          ))}
